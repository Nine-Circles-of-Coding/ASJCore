...
import static alexsocol.asjlib.preconfig.PreConfiguration.*;

@SuppressWarnings("ALL") // STFU - I'm not refactoring this shit
public class PreConfigCategory implements Map<String, PreProperty> {
	private String name;
	private String comment;
	private String languagekey;
	private ArrayList<PreConfigCategory> children = new ArrayList<PreConfigCategory>();
	private Map<String, PreProperty> properties = new TreeMap<String, PreProperty>();
	private int propNumber = 0;
	public final PreConfigCategory parent;
	...
	public PreConfigCategory(String name) {
		this(name, null);
	}
	
	public PreConfigCategory(String name, PreConfigCategory parent) {
		this.name = name;
		this.parent = parent;
		if (parent != null) {
	...
	public boolean equals(Object obj) {
		if (obj instanceof PreConfigCategory) {
			PreConfigCategory cat = (PreConfigCategory) obj;
	...
	public static String getQualifiedName(String name, PreConfigCategory parent) {
		return (parent == null ? name : parent.getQualifiedName() + PreConfiguration.CATEGORY_SPLITTER + name);
	}
	
	public PreConfigCategory getFirstParent() {
	...
	public Map<String, PreProperty> getValues() {
		return ImmutableMap.copyOf(properties);
	}
	
	public List<PreProperty> getOrderedValues() {
		if (this.propertyOrder != null) {
			ArrayList<PreProperty> set = new ArrayList<PreProperty>();
	...
	public PreConfigCategory setConfigEntryClass(Class<? extends IConfigEntry> clazz) {
	...
	public PreConfigCategory setLanguageKey(String languagekey) {
	 ...
	 */
	public PreConfigCategory setRequiresWorldRestart(boolean requiresWorldRestart) {
	 ...
	 * Defaults to true.
	 */
	public PreConfigCategory setShowInGui(boolean showInGui) {
	 ...
	 */
	public PreConfigCategory setRequiresMcRestart(boolean requiresMcRestart) {
	...
	public PreConfigCategory setPropertyOrder(List<String> propertyOrder) {
	...
	public PreProperty get(String key) {
	...
	public void write(BufferedWriter out, int indent) throws IOException {
		...
		PreProperty[] props = getOrderedValues().toArray(new PreProperty[]{});
		
		for (int x = 0; x < props.length; x++) {
			PreProperty prop = props[x];
		...
		for (PreConfigCategory child : children) {
	...
	public boolean hasChanged() {
		...
		for (PreProperty prop : properties.values()) {
	...
	@Override
	public PreProperty get(Object key) {
		return properties.get(key);
	}
	
	@Override
	public PreProperty put(String key, PreProperty value) {
		changed = true;
	...
	@Override
	public PreProperty remove(Object key) {
		changed = true;
		return properties.remove(key);
	}
	
	@Override
	public void putAll(Map<? extends String, ? extends PreProperty> m) {
		changed = true;
		if (this.propertyOrder != null)
	...
	@Override
	public Collection<PreProperty> values() {
		return properties.values();
	}
	
	@Override //Immutable copy, changes will NOT be reflected in this category
	public Set<Entry<String, PreProperty>> entrySet() {
		return ImmutableSet.copyOf(properties.entrySet());
	}
	
	public Set<PreConfigCategory> getChildren() {
		return ImmutableSet.copyOf(children);
	}
	
	public void removeChild(PreConfigCategory child) {
