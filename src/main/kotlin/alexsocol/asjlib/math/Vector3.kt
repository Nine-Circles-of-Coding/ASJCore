...
class Vector3 : Serializable {
  ...
  val I
    get() = arrayOf(x.I, y.I, z.I)
  val F
    get() = arrayOf(x.F, y.F, z.F)
  ...
  fun dotProduct(vec: Vector3): Double {
    ...
    if (d > 1 && d < 1.00001) d = 1.0 else if (d < -1 && d > -1.00001) d = -1.0
    return d
  }
  ...
  fun isInside(aabb: AxisAlignedBB) =
      x >= aabb.minX &&
          y >= aabb.maxY &&
          z >= aabb.minZ &&
          x < aabb.maxX &&
  ...
  @SideOnly(Side.CLIENT) fun vector3f() = Vector3f(x.F, y.F, z.F)

  @SideOnly(Side.CLIENT) fun vector4f() = Vector4f(x.F, y.F, z.F, 1f)
  ...
  companion object {
    ...
    @Transient val fallback = Vector3(-1, -1, -1)

    @Transient val zero = Vector3()

    @Transient val one = Vector3(1, 1, 1)

    @Transient val center = Vector3(0.5, 0.5, 0.5)

    @Transient val oX = Vector3(1, 0, 0)

    @Transient val oY = Vector3(0, 1, 0)

    @Transient val oZ = Vector3(0, 0, 1)
    ...
    fun fromTileEntityCenter(e: TileEntity) =
        Vector3(e.xCoord + 0.5, e.yCoord + 0.5, e.zCoord + 0.5)

    fun vecDistance(v1: Vector3, v2: Vector3) =
        sqrt((v1.x - v2.x).pow(2) + (v1.y - v2.y).pow(2) + (v1.z - v2.z).pow(2))

    fun vecEntityDistance(v: Vector3, e: Entity) =
        sqrt((v.x - e.posX).pow(2) + (v.y - e.posYp).pow(2) + (v.z - e.posZ).pow(2))

    fun vecTileDistance(v: Vector3, te: TileEntity) =
        sqrt(
            (v.x - te.xCoord + 0.5).pow(2) +
                (v.y - te.yCoord + 0.5).pow(2) +
                (v.z - te.zCoord + 0.5).pow(2))

    fun entityTileDistance(e: Entity, te: TileEntity) =
        sqrt(
            (e.posX - te.xCoord + 0.5).pow(2) +
                (e.posYp - te.yCoord + 0.5).pow(2) +
                (e.posZ - te.zCoord + 0.5).pow(2))

    fun entityDistance(e1: Entity, e2: Entity) =
        sqrt((e1.posX - e2.posX).pow(2) + (e1.posYp - e2.posYp).pow(2) + (e1.posZ - e2.posZ).pow(2))

    fun entityDistancePlane(e1: Entity, e2: Entity) = hypot(e1.posX - e2.posX, e1.posZ - e2.posZ)

    fun pointDistancePlane(x1: Number, y1: Number, x2: Number, y2: Number) =
        hypot(x1.D - x2.D, y1.D - y2.D)

    fun pointDistanceSpace(x1: Number, y1: Number, z1: Number, x2: Number, y2: Number, z2: Number) =
        sqrt((x1.D - x2.D).pow(2) + (y1.D - y2.D).pow(2) + (z1.D - z2.D).pow(2))

    private val Entity.posYp
      get() = posY - if (worldObj.isRemote && mc.thePlayer === this) 1.62 else 0.0
  }
}
