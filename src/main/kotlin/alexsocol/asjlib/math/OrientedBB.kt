...
/**
 * Oriented Bound Box: scalable, translatable, rotatable!<br> Collision calculations by
 * [@gszauer](https://github.com/gszauer/GamePhysicsCookbook) Useful scheme:
 *
 * h----g /| /| e----f | | d--|-c |/ |/ a----b
 *
 * Y | Z | / |/_____X
...
open class OrientedBB() {
  ...
  constructor(
      ...
      getBoundingBox(
          length.D / -2, height.D / -2, width.D / -2, length.D / 2, height.D / 2, width.D / 2))

  /** Returns array of vertices for this BB */
  fun vertices() = arrayOf(a, b, c, d, e, f, g, h)

  fun fromParams(length: Number, height: Number, width: Number) =
      fromAABB(
          getBoundingBox(
              length.D / -2, height.D / -2, width.D / -2, length.D / 2, height.D / 2, width.D / 2))
  ...
  /**
   * Rotates BB on given angle in DEG around given axis (axis coords are local, starting at pos[0,
   * 0, 0])
   */
  ...
  companion object {
    ...
    protected fun getInterval(obb: OrientedBB, axis: Vector3): Interval {
      ...
      vertex[0] =
          c.copy().add(a[0].copy().mul(e.x)).add(a[1].copy().mul(e.y)).add(a[2].copy().mul(e.z))
      vertex[1] =
          c.copy().sub(a[0].copy().mul(e.x)).add(a[1].copy().mul(e.y)).add(a[2].copy().mul(e.z))
      vertex[2] =
          c.copy().add(a[0].copy().mul(e.x)).sub(a[1].copy().mul(e.y)).add(a[2].copy().mul(e.z))
      vertex[3] =
          c.copy().add(a[0].copy().mul(e.x)).add(a[1].copy().mul(e.y)).sub(a[2].copy().mul(e.z))
      vertex[4] =
          c.copy().sub(a[0].copy().mul(e.x)).sub(a[1].copy().mul(e.y)).sub(a[2].copy().mul(e.z))
      vertex[5] =
          c.copy().add(a[0].copy().mul(e.x)).sub(a[1].copy().mul(e.y)).sub(a[2].copy().mul(e.z))
      vertex[6] =
          c.copy().sub(a[0].copy().mul(e.x)).add(a[1].copy().mul(e.y)).sub(a[2].copy().mul(e.z))
      vertex[7] =
          c.copy().sub(a[0].copy().mul(e.x)).sub(a[1].copy().mul(e.y)).add(a[2].copy().mul(e.z))
    ...
    fun getAABBSize(aabb: AxisAlignedBB) =
        Vector3(
            sqrt((aabb.minX - aabb.maxX).pow(2.0)) / 2.0,
            sqrt((aabb.minY - aabb.maxY).pow(2.0)) / 2.0,
            sqrt((aabb.minZ - aabb.maxZ).pow(2.0)) / 2.0)
  }
}
