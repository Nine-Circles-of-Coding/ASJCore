...
abstract class EntityRidable(world: World) : EntityCreature(world) {
  ...
  override fun attackEntityFrom(src: DamageSource, dmg: Float) =
      if (rider != null && src.entity === rider) false else super.attackEntityFrom(src, dmg)
  ...
  override fun onLivingUpdate() {
    rider =
        if (riddenByEntity != null && riddenByEntity is EntityPlayer) riddenByEntity as EntityPlayer
        else null
  ...
  override fun jump() {
    ...
    getActivePotionEffect(Potion.jump.id)?.let { motionY += (it.amplifier + 1) * 0.1 }
