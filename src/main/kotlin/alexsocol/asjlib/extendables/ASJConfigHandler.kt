...
abstract class ASJConfigHandler {
  ...
  /** Function called once in [FMLPreInitializationEvent] to initialize categories and properties */
  ...
  fun loadProp(
      ...
      restart: Boolean,
      desc: String?,
      ensureLength: Boolean = true
  ): IntArray {
    val prop = config.get(category, propName, default, desc)
    prop.setRequiresMcRestart(restart)
    return prop.intList.also {
      if (ensureLength && it.size < default.size)
          throw IllegalArgumentException(
              "Array $propName is not of suitable length (${it.size}), must be at least ${default.size}")
    }
  }
