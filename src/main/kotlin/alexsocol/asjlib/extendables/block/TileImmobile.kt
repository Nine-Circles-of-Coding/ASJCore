...
open class TileImmobile : ASJTile() {
  ...
  override fun updateEntity() {
    if (ASJUtilities.isServer &&
        lock != Lock(xCoord, yCoord, zCoord, worldObj.provider.dimensionId))
  ...
  private data class Lock(
    ...
    companion object {
      ...
      fun load(nbt: NBTTagCompound) =
          Lock(
              nbt.getInteger(TAG_C),
              nbt.getInteger(TAG_D),
              nbt.getInteger(TAG_E),
              nbt.getInteger(TAG_A),
              nbt.getInteger(TAG_B))
    }
  }
