...
class BlockPattern(
    ...
    tab: CreativeTabs?,
    lightlvl: Float = 0f,
    lightOpacity: Int = 255,
    hardness: Float = 1f,
    harvTool: String = "pickaxe",
    harvLvl: Int = 1,
    resistance: Float = 5f,
    sound: SoundType? = ASJUtilities.soundFromMaterial(material),
    private val isOpaque: Boolean = true,
    private val isBeacon: Boolean = false,
    private val isFalling: Boolean = false
  ...
  fun func_149830_m(world: World, x: Int, ay: Int, z: Int) {
    ...
    if (func_149831_e(world, x, y - 1, z) && y >= 0) {
      ...
      if (!fallInstantly &&
          world.checkChunksExist(x - b0, y - b0, z - b0, x + b0, y + b0, z + b0)) {
        if (!world.isRemote) {
          val efb =
              EntityFallingBlock(
                  world, x + 0.5, y + 0.5, z + 0.5, this, world.getBlockMetadata(x, y, z))
          func_149829_a(efb)
          efb.spawn()
        }
      } else {
