...
open class BlockModFence(texture: String, mat: Material, val gate: Block?) :
    BlockFence(texture, mat) {
  ...
  override fun canConnectFenceTo(world: IBlockAccess, x: Int, y: Int, z: Int): Boolean {
    ...
    return if (block is BlockFence) true
    else if (block !== this && block !== gate)
        if (block.material.isOpaque && block.renderAsNormalBlock())
