...
object ASJUtilities {
   ...
   */
  @JvmStatic fun getBlockName(block: Block) = block.unlocalizedName.substring(5)

  /**
   * Returns the name of the item
   * @param item Item to get name from
   */
  @JvmStatic fun getItemName(item: Item) = item.unlocalizedName.substring(5)

  /**
   * Registers block by name
   * @param block Block to register
   */
  @JvmStatic fun register(block: Block) = GameRegistry.registerBlock(block, getBlockName(block))!!

  /**
   * Registers item by name
   * @param item Item to register
   */
  @JvmStatic fun register(item: Item) = GameRegistry.registerItem(item, getItemName(item))
  ...
  fun sendToDimensionWithoutPortal(target: Entity, dimTo: Int, x: Double, y: Double, z: Double) {
    ...
    if (dimTo == target.dimension) {
      if (target is EntityLivingBase) return target.setPositionAndUpdate(x, y, z)
    ...
    if (target is EntityPlayerMP)
        return server.configurationManager.transferPlayerToDimension(
            target, dimTo, FreeTeleporter(worldTo, x, y, z))
    ...
    server.configurationManager.transferEntityToWorld(
        target, dimFrom, worldFrom, worldTo, FreeTeleporter(worldTo, x, y, z))
  ...
  fun dispatchTEToNearbyPlayers(tile: TileEntity) {
    ...
    for (player in players) if (player is EntityPlayerMP &&
        Vector3.pointDistancePlane(player.posX, player.posZ, tile.xCoord + 0.5, tile.zCoord + 0.5) <
            64)
  ...
  @JvmStatic
  fun isOre(stack: ItemStack?, name: String) =
      OreDictionary.getOreIDs(stack).any { it == OreDictionary.getOreID(name) }

  @JvmStatic
  fun removeRecipe(block: Block, stackSize: Int = 1, meta: Int = 0) =
      removeRecipe(ItemStack(block, stackSize, meta))

  @JvmStatic
  fun removeRecipe(item: Item, stackSize: Int = 1, meta: Int = 0) =
      removeRecipe(ItemStack(item, stackSize, meta))
  ...
  fun isNotInFieldOfVision(target: Entity, observer: EntityLivingBase): Boolean {
    ...
    val flag1 =
        yawFOVMin < 0f && (rotationYawPrime >= yawFOVMin + 360f || rotationYawPrime <= yawFOVMax) ||
            yawFOVMax >= 360f &&
                (rotationYawPrime <= yawFOVMax - 360f || rotationYawPrime >= yawFOVMin) ||
            yawFOVMax < 360f &&
                yawFOVMin >= 0f &&
                rotationYawPrime <= yawFOVMax &&
                rotationYawPrime >= yawFOVMin
    val flag2 =
        pitchFOVMin <= -180f &&
            (rotationPitchPrime >= pitchFOVMin + 360f || rotationPitchPrime <= pitchFOVMax) ||
            pitchFOVMax > 180f &&
                (rotationPitchPrime <= pitchFOVMax - 360f || rotationPitchPrime >= pitchFOVMin) ||
            pitchFOVMax < 180f &&
                pitchFOVMin >= -180f &&
                rotationPitchPrime <= pitchFOVMax &&
                rotationPitchPrime >= pitchFOVMin
  ...
  fun getMouseOver(
    ...
    val vec3 =
        Vec3.createVectorHelper(
            entity.posX, if (isClient) entity.posY else entity.posY + entity.eyeHeight, entity.posZ)
    ...
    val list =
        getEntitiesWithinAABB(
            ...
            Entity::class.java,
            entity.boundingBox
                .addCoord(vec31.xCoord * dist, vec31.yCoord * dist, vec31.zCoord * dist)
                .expand(f1.D, f1.D, f1.D))
    list.remove(entity)
    var d2 = d1
  ...
  private fun rayTrace(entity: EntityLivingBase, dist: Double): MovingObjectPosition? {
    ...
    val vec3 =
        Vec3.createVectorHelper(
            entity.posX, if (isServer) entity.posY + entity.eyeHeight else entity.posY, entity.posZ)
    val vec31 = entity.lookVec
    val vec32 = vec3.addVector(vec31.xCoord * dist, vec31.yCoord * dist, vec31.zCoord * dist)
  ...
  fun getPosition(target: EntityLivingBase): Vec3 {
    ...
    if (target.worldObj.isRemote) {
      ...
      if (target.isSneaking) v.yCoord -= 0.08
    }
  ...
  /**
   * @return Closest vulnerable player to entity within the given radius, ignoring invisibility and
   * sneaking<br></br> Can be null if none is found
   */
  @JvmStatic
  ...
  /**
   * @return Closest vulnerable player to coords within the given radius, ignoring invisibility and
   * sneaking<br></br> Can be null if none is found
   */
  @JvmStatic
  fun getClosestVulnerablePlayer(
      world: World,
      x: Double,
      y: Double,
      z: Double,
    ...
    for (i in world.playerEntities.indices) {
      ...
      if (!entityplayer1.capabilities.disableDamage && entityplayer1.isEntityAlive) {
        ...
        if ((distance < 0.0 || dist < distance * distance) &&
            (prevDist == -1.0 || dist < prevDist)) {
          prevDist = dist
  ...
  fun registerEntity(entityClass: Class<out Entity>, name: String, id: Int) {
    EntityRegistry.registerModEntity(
        entityClass, name, id, Loader.instance().activeModContainer().mod, 128, 1, true)
  }
  ...
  @JvmStatic
  fun chance(percent: Number) =
      if (percent.D <= 0.0) false
      else if (percent.D >= 100.0) true else Math.random() * 100 < percent.D

  /** @return String which tolds you to hold shift-key */
  @JvmStatic
  fun holdShift() =
      StatCollector.translateToLocal("tooltip.hold") +
          EnumChatFormatting.WHITE +
          " SHIFT " +
          EnumChatFormatting.GRAY +
          StatCollector.translateToLocal("tooltip.shift")
  ...
  fun holdCtrl() =
      StatCollector.translateToLocal("tooltip.hold") +
          ...
          " CTRL " +
          EnumChatFormatting.GRAY +
          StatCollector.translateToLocal("tooltip.ctrl")

  /** @return String which tolds you that this block/item/stuff is only for creative use */
  @JvmStatic fun creativeOnly() = StatCollector.translateToLocal("tooltip.creativeonly")!!
  ...
  fun registerDimension(id: Int, provider: Class<out WorldProvider>, keepLoaded: Boolean) {
    require(DimensionManager.registerProviderType(id, provider, keepLoaded)) {
      String.format("Failed to register provider for id %d, One is already registered", id)
    }
    DimensionManager.registerDimension(id, id)
  }

  /**
   * Generates **ore** into the world Args: block, it's meta, block to replace, world, rng, x, z
   * positions, min, max blocks in one place, min, max block groups in chunk, chance to be
   * generated, min, max Y-level
   */
  @JvmStatic
  fun generateOre(
      ore: Block,
      ...
      minVeinSize: Int,
      maxVeinSize: Int,
      minVeinsPerChunk: Int,
      maxVeinsPerChunk: Int,
      chanceToSpawn: Int,
      minY: Int,
      maxY: Int
  ) {
    ...
    for (i in 0 until veins) {
      ...
      WorldGenMinable(ore, meta, minVeinSize + rand.nextInt(maxVeinSize - minVeinSize + 1), replace)
          .generate(world, rand, posX, posY, posZ)
    }
  }

  /**
   * Fills holes of worldgen (no more structures in mid-air) Args: world, filler, x start, x end, y
   * to start from, z start, z end
   */
  @JvmStatic
  fun fillGenHoles(
      world: World,
    ...
    while (j >= 0) {
      for (i in 0 until radius * 2 + 1) {
        for (k in 0 until radius * 2 + 1) {
          ...
          if (Vector3.pointDistancePlane(xp, zp, centerX, centerZ).mfloor() <= radius - 1 &&
              isBlockReplaceable(world.getBlock(xp, j, zp)))
  ...
  private val replaceableMaterials =
      arrayOf(air, cactus, coral, fire, gourd, leaves, lava, plants, vine, water, web)
  ...
  fun soundFromMaterial(mat: Material) =
      when (mat) {
        ...
        sponge,
        tnt,
        web -> Block.soundTypeCloth
        glass,
        ice,
        packedIce,
        portal -> Block.soundTypeGlass
        cactus,
        coral,
        grass,
        leaves,
        plants,
        vine -> Block.soundTypeGrass
        ground,
        clay -> Block.soundTypeGravel
  ...
  private fun time(world: World?) =
      "[${format.format(world?.let { it.totalWorldTime % 1000 } ?: 0)}]"

  @JvmStatic
  fun chatLog(message: String) {
    val world =
        if (isServer) MinecraftServer.getServer()?.entityWorld
        else Minecraft.getMinecraft()?.theWorld
  ...
  @JvmStatic
  fun worldInfoForLog(world: World?) =
      "${time(world)} ${if (world?.isRemote == true) "[C]" else "[S]"}"
  ...
  private fun moddedLog(level: Level, message: String, e: Throwable? = null) {
    ...
    if (e == null) FMLRelaunchLog.log(modid, level, message)
    else FMLRelaunchLog.log(modid, level, e, message)
  }
  ...
  fun say(sender: ICommandSender?, message: String, vararg format: Any) {
    ...
    log(
        "[${sender.commandSenderName}!] ${StatCollector.translateToLocalFormatted(message, *format)}")
  }
  ...
  fun sayToAllOnline(message: String, vararg format: Any) {
    ...
    MinecraftServer.getServer()
        ...
        .sendChatMsg(ChatComponentTranslation(message, *format))
  ...
  fun sayToAllOPs(message: String) {
    ...
    MinecraftServer.getServer().configurationManager.playerEntityList.forEach {
      if ((it as EntityPlayer).commandSenderName in ops) say(it, message)
    }
    log(message)
  }
  ...
  fun toString(nbt: NBTTagList): String {
    ...
    for (obj in nbt.tagList) if (obj is NBTTagList || obj is NBTTagCompound) {
      val ts = if (obj is NBTTagList) toString(obj) else toString(obj as NBTTagCompound)
      for (s in ts.split("\n".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()) sb.append(
              "    ")
          .append(s)
          .append("\n")
