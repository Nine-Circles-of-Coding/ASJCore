...
class ASJPacketCompleter : IClassTransformer {
  ...
  private fun makeFromBytes(cl: ClassNode, callCustom: Boolean) {
    ...
    if (callCustom) {
      ...
      mv.visitMethodInsn(
          INVOKEVIRTUAL, cl.name, "fromCustomBytes", "(Lio/netty/buffer/ByteBuf;)V", false)
    }
    for (fn in getFileds(cl)) {
      ...
      mv.visitMethodInsn(
          INVOKESTATIC,
          "alexsocol/asjlib/network/ASJPacket",
          "read" + fn.desc.replace("/".toRegex(), "").replace(";".toRegex(), ""),
          "(Lio/netty/buffer/ByteBuf;)" + fn.desc,
          false)
  ...
  private fun makeToBytes(cl: ClassNode, callCustom: Boolean) {
    ...
    if (callCustom) {
      ...
      mv.visitVarInsn(ALOAD, 1)
      mv.visitMethodInsn(
          INVOKEVIRTUAL, cl.name, "toCustomBytes", "(Lio/netty/buffer/ByteBuf;)V", false)
    }
    for (fn in getFileds(cl)) {
      if (!descriptors.contains(fn.desc)) continue
      mv.visitVarInsn(ALOAD, 1)
      ...
      mv.visitMethodInsn(
          ...
          "alexsocol/asjlib/network/ASJPacket",
          "write",
          "(Lio/netty/buffer/ByteBuf;" + fn.desc + ")V",
          false)
    }
    mv.visitInsn(RETURN)
    mv.visitEnd()
