...
object ItemNBTHelper {
  ...
  /**
   * Tries to initialize an NBT Tag Compound in an ItemStack, this will not do anything if the stack
   * already has a tag compound
   */
  ...
  fun getByteArray(
      stack: ItemStack?,
      tag: String,
      defaultExpected: ByteArray = ByteArray(0)
  ): ByteArray {
    return if (verifyExistance(stack, tag)) stack!!.tagCompound.getByteArray(tag)
    else defaultExpected
  }
  ...
  fun getIntArray(
      stack: ItemStack?,
      tag: String,
      defaultExpected: IntArray = IntArray(0)
  ): IntArray {
    return if (verifyExistance(stack, tag)) stack!!.tagCompound.getIntArray(tag)
    else defaultExpected
  }
  ...
  fun getCompound(stack: ItemStack?, tag: String): NBTTagCompound {
    return if (verifyExistance(stack, tag)) stack!!.tagCompound.getCompoundTag(tag)
    else NBTTagCompound()
  }
  ...
  fun getCompound(stack: ItemStack?, tag: String, nullifyOnFail: Boolean): NBTTagCompound? {
    return if (verifyExistance(stack, tag)) stack!!.tagCompound.getCompoundTag(tag)
    else if (nullifyOnFail) null else NBTTagCompound()
  }
  ...
  fun getList(stack: ItemStack?, tag: String, objtype: Int): NBTTagList {
    return if (verifyExistance(stack, tag)) stack!!.tagCompound.getTagList(tag, objtype)
    else NBTTagList()
  }
  ...
  fun getList(stack: ItemStack?, tag: String, objtype: Int, nullifyOnFail: Boolean): NBTTagList? {
    return if (verifyExistance(stack, tag)) stack!!.tagCompound.getTagList(tag, objtype)
    else if (nullifyOnFail) null else NBTTagList()
  }
}
