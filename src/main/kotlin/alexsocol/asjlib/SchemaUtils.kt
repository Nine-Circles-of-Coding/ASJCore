...
object SchemaUtils {
  ...
  fun generate(world: World, x: Int, y: Int, z: Int, schemaText: String) {
    ...
    for (ele in arr) {
      ...
      for (loc in ele.location) {
        ...
        if (loc.nbt != null) {
          val tile =
              TileEntity.createAndLoadEntity(JsonToNBT.func_150315_a(loc.nbt) as NBTTagCompound)
                  ?: return
          tile.xCoord = x + loc.x
          tile.yCoord = y + loc.y
          tile.zCoord = z + loc.z
  ...
  fun checkStructure(
      ...
      z: Int,
      structure: String,
      onFail: ((Int, Int, Int, Int) -> Unit)? = null
  ): Boolean {
    ...
    for (ele in arr) {
      for (loc in ele.location) {
        ...
        fun check(): Boolean {
          if (world.getBlock(i, j, k) != Block.getBlockFromName(ele.block) ||
              world.getBlockMetadata(i, j, k) != loc.meta)
              return false
  ...
  fun loadStructure(path: String): String {
    return javaClass.getResourceAsStream("/assets/$path")!!.use { it.readBytes().decodeToString() }
  }
}
