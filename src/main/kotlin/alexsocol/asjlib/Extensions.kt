...
fun <T> Collection<T>.random(random: Random = Random()) =
 ...
 *
 * If there are odd amount of elements - then last pair will have either last element to [last]
 * argument or same element in both positions if [last] is null/unprovided
 */
...
fun <E : Any> getEntitiesWithinAABB(
    world: World,
    clazz: Class<E>,
    aabb: AxisAlignedBB
): ArrayList<E> = world.getEntitiesWithinAABB(clazz, aabb) as ArrayList<E>

@Suppress("UNCHECKED_CAST") // FUCKING RAW GENERICS
fun <E : Any> selectEntitiesWithinAABB(
    world: World,
    clazz: Class<E>,
    aabb: AxisAlignedBB,
    selector: (E) -> Boolean
  ...
  class GenericLambdaEntitySelector<E>(val selector: (E) -> Boolean) : IEntitySelector {
    ...
    override fun isEntityApplicable(entity: Entity) = selector(entity as E)
  }

  return world.selectEntitiesWithinAABB(clazz, aabb, GenericLambdaEntitySelector(selector))
      as ArrayList<E>
}

fun getBoundingBox(x: Number, y: Number, z: Number) = getBoundingBox(x, y, z, x, y, z)

fun getBoundingBox(x1: Number, y1: Number, z1: Number, x2: Number, y2: Number, z2: Number) =
    AxisAlignedBB.getBoundingBox(x1.D, y1.D, z1.D, x2.D, y2.D, z2.D)!!

fun TileEntity.boundingBox(range: Number = 0) =
    getBoundingBox(xCoord, yCoord, zCoord, xCoord + 1, yCoord + 1, zCoord + 1).expand(range)

fun Entity.boundingBox(range: Number = 0) =
    getBoundingBox(posX, posY, posZ, posX + width, posY + height, posZ + width)
        .offset(width / -2.0, 0.0, width / -2.0)
        .expand(range)
...
fun Entity.setPosition(e: Entity, oX: Double = 0.0, oY: Double = 0.0, oZ: Double = 0.0) =
    setPosition(e.posX + oX, e.posY + oY, e.posZ + oZ)

fun Entity.setPosition(c: ChunkCoordinates, oX: Double = 0.0, oY: Double = 0.0, oZ: Double = 0.0) =
    setPosition(c.posX + oX, c.posY + oY, c.posZ + oZ)
...
fun EntityPlayerMP.hasAchievement(a: Achievement?) =
    if (a == null) false else func_147099_x().hasAchievementUnlocked(a)

fun EntityPlayer.hasAchievement(a: Achievement?) =
    if (this is EntityPlayerMP) hasAchievement(a)
    else if (this is EntityClientPlayerMP) hasAchievement(a) else false
...
fun ItemStack.areItemStackTagsEqual(stack: ItemStack): Boolean {
  return when {
    stackTagCompound == null || stackTagCompound.hasNoTags() ->
        stack.stackTagCompound == null || stack.stackTagCompound.hasNoTags()
    stack.stackTagCompound == null || stack.stackTagCompound.hasNoTags() ->
        stackTagCompound == null || stackTagCompound.hasNoTags()
...
fun PotionEffectU(id: Int, time: Int, lvl: Int = 0, ambient: Boolean = false) =
    PotionEffect(id, time, lvl, ambient).apply { curativeItems.clear() }
