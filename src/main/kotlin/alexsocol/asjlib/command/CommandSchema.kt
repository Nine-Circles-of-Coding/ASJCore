...
object CommandSchema : CommandBase() {
  ...
  override fun getCommandUsage(sender: ICommandSender?) =
      "/$commandName <pos1> | <pos2> | <save> [filler] | <load> <filename>"
  ...
  override fun processCommand(sender: ICommandSender, args: Array<String>) {
    ...
    try {
      ...
      when (args[0]) {
        ...
        "save" -> {
          if (poses[name]!!.left == null)
              throw WrongUsageException("asjcore.commands.schema.noPosN", 1)
          if (poses[name]!!.right == null)
              throw WrongUsageException("asjcore.commands.schema.noPosN", 2)
  ...
  override fun addTabCompletionOptions(
    ...
    return when (args.size) {
      ...
      2 ->
          if (args[0] == "save")
              getListOfStringsFromIterableMatchingLastWord(args, Block.blockRegistry.keys)
          else null
      else -> null
    }
  }
  ...
  fun getNewFile(file: Int = 0): File {
    ...
    if (!e.exists()) e.createNewFile() else return getNewFile(file + 1)
            ...
            if (k != filler) {
              add(
                  JsonObject().apply {
                    ...
                    add(
                        "location",
                        JsonArray().apply { for (v in map[k].orEmpty()) add(v.getJson()) })
                  })
            }
          }
        })
