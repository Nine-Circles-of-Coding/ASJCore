...
class ResourceLocationAnimated : ResourceLocation {
  ...
  private constructor(mod: String, path: String) : super(mod, path) {
    init(
        getResourceSafe(ResourceLocation(resourceDomain, "$resourcePath.meta"))?.inputStream,
        mc.resourceManager.getResource(this).inputStream)
  }
  ...
  private fun init(metaStream: InputStream?, imageStream: InputStream): ResourceLocationAnimated {
    ...
    frameList =
        loadImage(
            getMeta(meta, MARKER_HEIGHT, 0),
            imageStream,
            getMeta(meta, MARKER_INTERPOLATION_STEPS, 1))
    return this
  }

  private fun getMeta(meta: List<String>?, marker: String, default: Int) =
      (meta?.firstOrNull { it.startsWith(marker) } ?: "$marker$default").replace(marker, "").toInt()
  ...
  private fun doInterpolation(
      prev: BufferedImage,
      cur: BufferedImage,
      frameList: ArrayList<Int>,
      step: Int,
      last: Boolean
  ) {
    ...
    for (i in 1 until step) {
      ...
      for (u in 0 until width) {
        for (v in 0 until height) {
          interstep.setRGB(
              u, v, interpolateColor(prev.getRGB(u, v), cur.getRGB(u, v), fraction * i))
        }
      }
  ...
  private fun interpolateChannel(channel1: Int, channel2: Int, fraction: Double) =
      ((channel2 - channel1) * fraction + channel1).I

  private fun put(frameList: ArrayList<Int>, part: BufferedImage) =
      frameList.add(
          TextureUtil.uploadTextureImageAllocate(GL11.glGenTextures(), part, false, false))

  fun getCurrentFrame() =
      frameList[((mc.theWorld.totalWorldTime % (framerate * frameList.size)) / framerate).I]
