...
object RenderPostShaders {
  ...
  fun dispatchObjects() {
    ...
    if (pt != null) {
      if (pt is ResourceLocationAnimated) pt.bind()
      else Minecraft.getMinecraft().renderEngine.bindTexture(pt)
    }
    ...
    for (obj in shaders) {
      ...
      if (ot != null && ot !== prevObj.texture) {
        if (ot is ResourceLocationAnimated) ot.bind()
        else Minecraft.getMinecraft().renderEngine.bindTexture(ot)
      }
