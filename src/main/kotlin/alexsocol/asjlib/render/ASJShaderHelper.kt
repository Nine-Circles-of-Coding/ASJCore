...
object ASJShaderHelper {
  ...
  /**
   * Creates shader bundle for future using. Put your shaders to /assets/modid/.
  ...
  fun createProgram(vertLocation: String?, fragLocation: String?): Int {
    return try {
      ...
      if (crashOnError) throw e else 0
    }
  }
    ...
    try {
      ...
      if (glGetShaderi(shaderID, GL_COMPILE_STATUS) == GL_FALSE)
          throw RuntimeException(
              "Error Compiling shader [$filename]: " + getShaderLogInfo(shaderID))
  ...
  private fun readFileAsString(filename: String): String {
    return mc.resourceManager
        .getResource(ResourceLocation(Loader.instance().activeModContainer().modId, filename))
        .inputStream
        .readBytes()
        .decodeToString()
  }
  ...
  private val total
    get() = gameTicks + mc.timer.renderPartialTicks
