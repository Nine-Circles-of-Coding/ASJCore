...
open class RenderGlowingLayerBlock : ISimpleBlockRenderingHandler {
  ...
  override fun renderWorldBlock(
    ...
    renderAllSides(world, x, y, z, block, renderer, false) { s, m ->
      (block as IGlowingLayerBlock).getGlowIcon(s, m)
    }
  ...
  companion object {
    ...
    fun renderAllSides(
        ...
        renderer: RenderBlocks,
        allsides: Boolean,
        getIcon: (Int, Int) -> IIcon?
    ) {
      for ((i, d) in ForgeDirection.VALID_DIRECTIONS.withIndex()) if (allsides ||
          block.shouldSideBeRendered(world, x + d.offsetX, y + d.offsetY, z + d.offsetZ, i))
          getIcon.invoke(i, world.getBlockMetadata(x, y, z))?.let {
            renderers[i].invoke(renderer, block, x.D, y.D, z.D, it)
          }
    }
