...
object ASJRenderHelper {
  ...
  @JvmStatic fun addAlpha(color: Int, alpha: Int) = alpha and 0xFF shl 24 or (color and 0x00FFFFFF)
  ...
  @JvmStatic
  fun interpolate(prev: Double, current: Double) =
      prev + (current - prev) * mc.timer.renderPartialTicks

  /** Translates matrix to follow player (if something is bound to world's zero coords) */
  @JvmStatic
  fun interpolatedTranslation(entity: Entity) =
      glTranslated(
          interpolate(entity.lastTickPosX, entity.posX),
  ...
  @JvmStatic
  fun interpolateChannel(channel1: Int, channel2: Int, fraction: Double) =
