...
/** Kotlin Language Adapter Allows to make [Mod]-annotated objects */
class KotlinAdapter : ILanguageAdapter {
  ...
  override fun getNewInstance(
      ...
      instanceFactory: Method?
  ): Any {
    if (instanceFactory != null) return instanceFactory.invoke(null)

    ASJReflectionHelper.getField(modClass, "INSTANCE")?.apply {
      ASJReflectionHelper.getValue<Any>(this, null as Any?)
          ?.apply { // cast because of 'Overload resolution ambiguity'
            return this
          }
    }
  ...
  override fun setInternalProxies(container: ModContainer, side: Side, loader: ClassLoader) {
    ...
    modClass.declaredFields.forEach { field ->
      ...
      val proxyInstance =
          getNewInstance(null, Class.forName(targetType, true, loader), loader, null)
...
/**
 * Kotlin specific proxy annotation Allows proxies to be objects Works only in [Mod]-annotated
 * classes/objects
 */
