...
object PatcherEventHandler {
  ...
  fun disableDamageInCreative(e: LivingAttackEvent) {
    if (!PatcherConfigHandler.creativeDamage &&
        (e.entityLiving as? EntityPlayer)?.capabilities?.isCreativeMode == true)
        e.isCanceled = true
  }
  ...
  fun fixNaNHealthBug(e: LivingEvent.LivingUpdateEvent) {
    ...
    if (max.isNaN())
        max = target.getEntityAttribute(SharedMonsterAttributes.maxHealth).attributeValue.F
  ...
  fun onBreakingInAir(e: PlayerEvent.BreakSpeed) {
    if (!PatcherConfigHandler.flyFastDig ||
        e.entityPlayer.onGround ||
        !e.entityPlayer.capabilities.isFlying)
        return
    e.newSpeed *= 5f
  }
...
object PatcherEventHandlerClient {
  ...
  fun onItemTooltip(e: ItemTooltipEvent) {
    if (GuiScreen.isShiftKeyDown()) {
      ...
      if (PatcherConfigHandler.showOreDict)
          run {
            ...
            for (id in ids) e.toolTip.add(OreDictionary.getOreName(id))
          }
      ...
      if (stack.hasTagCompound() && e.showAdvancedItemTooltips) {
        ...
        e.toolTip.addAll(
            listOf(
                *ASJUtilities.toString(stack.tagCompound)
                    .split("\n".toRegex())
