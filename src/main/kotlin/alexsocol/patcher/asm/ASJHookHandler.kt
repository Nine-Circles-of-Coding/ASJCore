...
object ASJHookHandler {
  ...
  @Hook(
      ...
      createMethod = true,
      superClass =
          "net/minecraft/entity/effect/EntityWeatherEffect.${Opcodes.ALOAD}.1.(Lnet/minecraft/world/World;)V")
  ...
  fun EntityEnderman(thiz: EntityEnderman?) {
    val uuid =
        UUID.fromString(
            "020E0DFB-87AE-4653-9556-831010E291A0") // Entries in at.cfg causes gradle error
    ASJReflectionHelper.setStaticFinalValue(
        EntityEnderman::class.java, uuid, "attackingSpeedBoostModifierUUID", "field_110192_bp")
    ASJReflectionHelper.setStaticFinalValue(
        EntityEnderman::class.java,
        AttributeModifier(uuid, "Attacking speed boost", 6.199999809265137, 0).setSaved(false),
    ...
    arrayOf(
            ...
            Blocks.melon_block,
            Blocks.mycelium)
        .forEach { EntityEnderman.setCarriable(it, true) }
  }

  @JvmStatic
  @Hook(returnCondition = ReturnCondition.ON_TRUE)
  fun spawnEntityInWorld(world: World, target: Entity?): Boolean {
    if (target !is EntityWeatherEffect) return false
  ...
  fun `EntityList$clinit`(e: EntityList?) {
    ...
    if (PatcherConfigHandler.lightningID != -1)
        EntityList.addMapping(
            EntityLightningBolt::class.java, "LightningBolt", PatcherConfigHandler.lightningID)
  }
  ...
  fun processCommand(c: CommandSummon, sender: ICommandSender?, args: Array<String?>): Boolean {
    ...
    for (i in 0 until count.substring(1).toInt()) c.processCommand(sender, newArgs)
  ...
  fun readFromNBT(stack: ItemStack, nbt: NBTTagCompound): Boolean {
    ...
    if (nbt.hasKey("tag", 10)) stack.stackTagCompound = nbt.getCompoundTag("tag")
  ...
  private fun migrate(nbt: NBTTagCompound) {
    ...
    val stack =
        ItemStack(
            Item.getItemById(nbt.getShort("id").toInt()),
            nbt.getByte("Count").toInt(),
            max(0, nbt.getShort("Damage").toInt()))
  ...
  @Hook(
      ...
      returnType = "float",
      returnAnotherMethod = "armorNotApplied")
  fun ApplyArmor(
  ...
  fun addStats(stats: FoodStats, foodLevel: Int, foodSaturationLevel: Float) {
    ...
    nbt.setFloat(
        "foodSaturationLevel",
        min(stats.saturationLevel + e.newFoodLevel * e.newSaturationLevel * 2f, stats.foodLevel.F))
  ...
  @JvmStatic
  @Hook(returnCondition = ReturnCondition.ON_TRUE)
  fun canConnectFenceTo(fence: BlockFence, world: IBlockAccess, x: Int, y: Int, z: Int) =
  ...
  fun renderBlockWall(render: RenderBlocks, block: BlockWall, x: Int, y: Int, z: Int): Boolean {
    ...
    val doNotRenderPost =
        render.blockAccess.getBlock(x, y + 1, z) !is BlockWall &&
            render.blockAccess.getBlock(x, y + 1, z) !is BlockSkull &&
            render.blockAccess.getBlock(x, y - 1, z) !is BlockWall
  ...
  fun updatePotionEffects(e: EntityLivingBase) {
    try {
      ...
      while (iterator.hasNext()) {
        ...
        if (potioneffect.potionID < 0)
            throw IllegalArgumentException(
                "Potion ID is negative (${potioneffect.potionID}). Did you set some ID to 128+ without potion fixing mod?")

        if (potioneffect.potionID !in Potion.potionTypes.indices ||
            Potion.potionTypes[potioneffect.potionID] == null)
      ...
      if (e.potionsNeedUpdate) {
        if (!e.worldObj.isRemote) {
          if (e.activePotionsMap.isEmpty()) {
            ...
            e.dataWatcher.updateObject(
                8, (if (PotionHelper.func_82817_b(e.activePotionsMap.values)) 1 else 0).toByte())
      ...
      if (i > 0) {
        ...
        if (flag) {
          ...
          e.worldObj.spawnParticle(
              if (flag1) "mobSpellAmbient" else "mobSpell",
              e.posX + (e.worldObj.rand.nextDouble() - 0.5) * e.width.D,
              e.posY + e.worldObj.rand.nextDouble() * e.height.D - e.yOffset.D,
              e.posZ + (e.worldObj.rand.nextDouble() - 0.5) * e.width.D,
              d0,
              d1,
              d2)
        }
      ...
      ASJReflectionHelper.setValue(
          message_f,
          e,
          ASJReflectionHelper.getValue<String>(message_f, e) +
              "\nIt is possible that you got potion ID conflict. Try installing 'Extended Potions' or make sure you have all IDs BELOW 128!",
          true)
  ...
  private val message_f =
      ASJReflectionHelper.getField(java.lang.Throwable::class.java, "detailMessage")
  private val stackTrace_f =
      ASJReflectionHelper.getField(java.lang.Throwable::class.java, "stackTrace")
  ...
  fun extinguishFire(
    ...
    val breakable =
        if (player != null)
            block.getPlayerRelativeBlockHardness(player, world, i, j, k) > 0f ||
  ...
  val mergeItemStack by lazy {
    ASJReflectionHelper.getMethod(
            Container::class.java,
            arrayOf("mergeItemStack", "func_75135_a"),
            arrayOf(ItemStack::class.java, Int::class.java, Int::class.java, Boolean::class.java))
        ?.also { it.isAccessible = true }
  }

  @JvmStatic
  @Hook(returnCondition = ReturnCondition.ALWAYS)
  fun transferStackInSlot(
    ...
    if (slot != null && slot.hasStack) {
      ...
      if (slotID == 0) {
        ...
        if ((container.inventorySlots[0] as Slot).hasStack ||
            !(container.inventorySlots[0] as Slot).isItemValid(itemstack1))
            return null
  ...
  fun setupFog(renderer: EntityRenderer, fogMode: Int, renderPartialTicks: Float) {
    ...
    val creative =
        if (entitylivingbase is EntityPlayer) entitylivingbase.capabilities.isCreativeMode
        else false

    fun setFogColorBuffer(
        p_78469_1_: Float,
        p_78469_2_: Float,
        p_78469_3_: Float,
        p_78469_4_: Float
    ...
    if (fogMode == 999) {
      ...
      GL11.glFog(
          GL11.GL_FOG_COLOR,
          setFogColorBuffer(
              renderer.fogColorRed, renderer.fogColorGreen, renderer.fogColorBlue, 1f))
      GL11.glNormal3f(0f, -1f, 0f)
      GL11.glColor4f(1f, 1f, 1f, 1f)
      val block =
          ActiveRenderInfo.getBlockAtEntityViewpoint(
              renderer.mc.theWorld, entitylivingbase, renderPartialTicks)
      var f1: Float

      val event =
          EntityViewRenderEvent.FogDensity(
              renderer, entitylivingbase, block, renderPartialTicks.D, 0.1f)
      ...
      if (MinecraftForge.EVENT_BUS.post(event)) {
        ...
        if (entitylivingbase.isPotionActive(Potion.waterBreathing)) {
          ...
          GL11.glFogf(
              GL11.GL_FOG_DENSITY,
              if (PatcherConfigHandler.clearWater) 0.01f
              else 0.1f - EnchantmentHelper.getRespiration(entitylivingbase).F * 0.03f)
        }
        ...
        if (renderer.mc.theWorld.provider.worldHasVoidParticles &&
            PatcherConfigHandler.voidFog &&
            !creative) {
          var d0 =
              (entitylivingbase.getBrightnessForRender(renderPartialTicks) and 15728640 shr 20).D /
                  16.0 +
                  (entitylivingbase.lastTickPosY +
                      (entitylivingbase.posY - entitylivingbase.lastTickPosY) *
                          renderPartialTicks.D +
                      4.0) / 32.0
        ...
        if (renderer.mc.theWorld.provider.doesXZShowFog(
            entitylivingbase.posX.I, entitylivingbase.posZ.I)) {
        ...
        MinecraftForge.EVENT_BUS.post(
            EntityViewRenderEvent.RenderFogEvent(
                renderer, entitylivingbase, block, renderPartialTicks.D, fogMode, f1))
      }
  ...
  @JvmStatic
  @Hook(returnCondition = ReturnCondition.ON_TRUE)
  fun trackBrokenTexture(
      handler: FMLClientHandler,
  ...
  fun getArmSwingAnimationEnd(e: EntityLivingBase, @ReturnValue result: Int) =
      if (e is ICustomArmSwingEndEntity) e.getCustomArmSwingAnimationEnd() else result
    ...
    return try {
      if (field_150493_b.matches("[-+]?\\d*\\.?\\d+[dD]".toRegex())) {
        ...
        if (field_150493_b.startsWith("[") && field_150493_b.endsWith("]")) {
          if (field_150493_b.length > 2) {
            ...
            try {
              if (astring.size <= 1) {
                ...
                else NBTTagIntArray(intArrayOf(st.toInt()))
              } else {
                val st = astring[0].trim() // supposing that all other also endsWith b
                if (st.endsWith('b') || st.endsWith('B'))
                    NBTTagByteArray(
                        ByteArray(astring.size) { astring[it].trim().substringEnding(1).toByte() })
                else NBTTagIntArray(IntArray(astring.size) { astring[it].trim().toInt() })
              }
          ...
          if (field_150493_b_.startsWith("\"") &&
              field_150493_b_.endsWith("\"") &&
              field_150493_b_.length > 2) {
  ...
  //	@JvmStatic
  //	@Hook(returnCondition = ReturnCondition.ALWAYS)
  //	fun getFlag(entity: Entity, id: Int) = entity.dataWatcher.getWatchableObjectInt(0) and (1 shl
  // id) != 0
  //
  //	@JvmStatic
  //	@Hook(returnCondition = ReturnCondition.ALWAYS)
  ...
  fun func_151519_b(src: EntityDamageSource, victim: EntityLivingBase): IChatComponent {
    ...
    val itemstack =
        if (damageSourceEntity is EntityLivingBase) damageSourceEntity.heldItem else null
    val s = "death.attack." + src.damageType
    val s1 = "$s.item"
    val component = damageSourceEntity?.func_145748_c_() ?: ChatComponentText("null")
    return if (itemstack != null && itemstack.hasDisplayName() && StatCollector.canTranslate(s1))
        ChatComponentTranslation(s1, victim.func_145748_c_(), component, itemstack.func_151000_E())
    else ChatComponentTranslation(s, victim.func_145748_c_(), component)
  }
  ...
  @JvmStatic
  @Hook(injectOnExit = true)
  fun getCollidingBoundingBoxes(
      world: World,
      entity: Entity?,
      aabb: AxisAlignedBB?,
      @ReturnValue result: MutableList<AxisAlignedBB?>
  ): List<AxisAlignedBB?> {
