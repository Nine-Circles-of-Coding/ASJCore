...
class RealmsDeleteTransformer: IClassTransformer {
	...
	override fun transform(name: String?, transformedName: String?, basicClass: ByteArray?): ByteArray? {
		...
		if (transformedName == "com.mojang.realmsclient.gui.ChatFormatting" || transformedName == "net.minecraft.realms.RealmsSharedConstants") {
			ASJUtilities.warn("****************************************")
			...
			repeat(trace.size - 2) {
				ASJUtilities.warn("*  at ${trace[it+2]}")
			}
			ASJUtilities.warn("****************************************")
		...
		if (deleted) throw ClassNotFoundException("Realms were deleted by ASJCore. Disable deletion in configs if needed.")
