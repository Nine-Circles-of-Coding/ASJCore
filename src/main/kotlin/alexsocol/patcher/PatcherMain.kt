...
object PatcherMain {
  ...
  fun preInit(e: FMLPreInitializationEvent) {
    ...
    noItems.forEach { block ->
      GameData.getMain()
          .registerItem(
              ItemBlock(block),
              Block.blockRegistry.getNameForObject(block) + "_item",
  ...
  fun init(e: FMLInitializationEvent) {
    ...
    if (ASJUtilities.isClient) PatcherEventHandlerClient.eventForge()
  }

  @Mod.EventHandler
  fun onServerStarting(e: FMLServerStartingEvent) {
    PatcherConfigHandler.commands.forEach {
      e.registerServerCommand(Commands.valueOf(it).command())
    }
